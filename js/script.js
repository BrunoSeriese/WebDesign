...
$("#ru").click(function () {
  ...
  $("#design").html("<a>курс веб-дизайна</a>");
  $("#userinter").html("<a>веб-дизайн</a>");
  $("#contact-ons").html("<a>контакты</a>");
  $("#ons-projekt").html("<a>проекты</a>");
  $("#onze-info").html("<a>Информация</a>");


  //POPUP TWO
  $(".popup--2__text--1").text("Всегда хотели создавать собственные сайты?");
  $(".price__tag").text("в час");
  $(".price__disclaimer").text("(€10- 30 минут)");
  $(".popup--2__translate--1").text("Индивидуальный онлайн урок (на английском)");
  $(".popup--2__translate--2").text("Вы сможете создавать рабочие сайты уже после первого урока!");
  $(".popup--2__translate--3").text("Ответы на все интересующие вопросы");
  $(".popup--2__translate--4").text("При покупке пакета уроков предоставляется скидка");

  //POPUP THREE
  $(".popup--3__text--1").text("вы хотите:");
  $(".popup--3__translate--1").text("современный интерфейс?");
  $(".popup--3__translate--2").text("Сайт, который так же хорошо выглядит на телефоне?");
  $(".popup--3__translate--3").text("Сайт, который излучает профессионализм?");
  $(".popup--3__footer").text("Свяжитесь с нами для обсуждения деталей");
